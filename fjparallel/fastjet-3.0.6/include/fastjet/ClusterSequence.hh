...
////////////start of concurrent information  //**//
  ...
  template<class L> ClusterSequence (
			          const std::vector<L> & pseudojets,
				  const JetDefinition & jet_def,
				  const std::vector<Tile> & buffertiles, //**//
				  const bool & writeout_combinations = false);
/*
      TODO : figure out how to make concurrent ctor initialise:
                    s_multicore = true ;
                   (s_firstevent = variable)
      TODO : figure out how to make singlecore ctor initialise:
                    s_multicore = false ;
                    s_firstevent = true ;
      TODO : if s_firstevent exists in here, figure out how to
             let it know it's state (since it's not global)
*/
  ...
  bool s_bottom ;
  int s_flag ;
  boost::mutex s_topmtx ;
  boost::mutex s_bottommtx ;
  boost::mutex s_flagmtx ;
  ...
  void s_NN_INIT( Tile *tile ){//one thread per tile
    TiledJet * jetA ;
    TiledJet * jetB ;
    while(1){
      s_NOTIFY_MAIN() ;  
  ...
  void s_CREATE_THREADS() {
    for ( unsigned int i = 0 ; i < (*_tiles).size() ; ++i ) {
      ...
      boost::mutex * _mtxqaz ;
      s_tilelocks.push_back( _mtxqaz ) ;
      s_tilethreads.create_thread(boost::bind(&threadstruct::s_NN_INIT, tile));*/
    }
  }
